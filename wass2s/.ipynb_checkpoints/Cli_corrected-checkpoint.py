    ...
    if forecast_prob is not None:
        try:
            ...
            plot_prob_forecasts(
                ...
                forecast_prob.drop_vars('T').squeeze(), 
                f"{model_name} {season_str}-{forecast_year} IC: {calendar.month_name[int(month_init)]}", 
                country_code=country_iso3, source="gadm", admin_level=2,
...
def stage_analog(args, workdir: Path):
    ...
    rainfall = xr.load_dataarray(workdir / "rainfall_predictand.nc")
    out_dir = workdir / "forecasts"; out_dir.mkdir(exist_ok=True)
    model = WAS_Analog(dir_to_save=str(workdir / "analog"), 
                       year_start=1990,
                       # year_start=args.obs_years[0]-1, 
                       year_forecast=2025,
                       # year_forecast=args.forecast_year,
                       month_of_initialization=12, 
                       # month_of_initialization=int(args.init_month)-1,
                       predictor_vars=[{'reanalysis_name': 'NOAA', 'model_name': 'ECMWF_51', 'variable': 'SST', 'area': [40, -180, -35, 100]}], method_analog="bias_based", standardize=True, rolling=3, lead_time=[1, 2, 3, 4, 5], clim_year_start=args.clim_years[0], clim_year_end=args.clim_years[1], dist_method=args.dist_method)

    best_code_da, best_shape_da, best_loc_da, best_scale_da = load_bestfit_params(workdir)
    cv_k = {"best_code_da": best_code_da, "best_shape_da": best_shape_da, "best_loc_da": best_loc_da, "best_scale_da": best_scale_da} if args.dist_method=="bestfit" else {}
    
    cv = WAS_Cross_Validator(n_splits=len(rainfall.get_index("T")), nb_omit=2)
    hd, hp = cv.cross_validate(model, rainfall, clim_year_start=args.clim_years[0], clim_year_end=args.clim_years[1], **cv_k)
    _, fd, fp = model.forecast(rainfall, args.clim_years[0], args.clim_years[1], hd, **cv_k)

    scores_consolidated = {}
    was_verify = WAS_Verification(dist_method=args.dist_method)
    
    evaluate_and_plot_forecasts(
        was_verify, rainfall, hd, hp, fp, "Analog", 'fourth.approach', args.clim_years[0], args.clim_years[1], 
        scores_consolidated, str(workdir/"scores"), str(out_dir), season_to_str(args.season_months), 
        args.forecast_year, args.init_month, FCST_LABELS_FR, args.country_code, args.logo
    )
    
    (workdir/"intermediate").mkdir(exist_ok=True, parents=True)
    xr.Dataset({"hdcst_det_fourth.approach": hd, "hdcst_prob_fourth.approach": hp}).to_netcdf(workdir/"intermediate/analog_hindcasts.nc")
    xr.Dataset({"fcst_det_fourth.approach": fd, "fcst_prob_fourth.approach": fp}).to_netcdf(workdir/"intermediate/analog_forecasts.nc")
    save_scores(workdir, scores_consolidated, "scores_consolidated_analog.pkl")

    mark_done(check_dir, "analog", {"note": "analogue completed"})
...
def main():
    ...
    stages = {
        ...
        'cca-gcm-prcp':  lambda: _execute_cca_pipeline(args, workdir, downloader, "PRCP", args.sst_extent, {'A': ('A', -26, 25, 4, 25)}, "cca_gcm-prcp", top_n=5),
        ...
        'cca-gcm-vwind': lambda: _execute_cca_pipeline(args, workdir, downloader, "VGRD_850", args.wind_extent, {'A': ('A', -60, 30, -50, 50)}, "cca_gcm_vwind", top_n=3),
